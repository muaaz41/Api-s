  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({
          error: "No airline found with the given IATA and ICAO codes.",
        });
    }
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({
          error: "Airline not found with the given IATA and ICAO codes.",
        });
    }
...
app.get("/airports/country/:country", async (req, res) => {
  try {
    const { country } = req.params;
    const result = await pool.query(
      "SELECT * FROM airports WHERE country = $1",
      [country]
    );
    res.json(result.rows);
  } catch (err) {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({
          error: "No airport found with the given IATA and ICAO codes.",
        });
    }
  ...
  try {
    ...
    if (result.rowCount === 0) {
      return res
        .status(404)
        .json({
          error: "Airport not found with the given IATA and ICAO codes.",
        });
    }
...
app.get("/routes/:departure/:arrival", async (req, res) => {
  try {
    ...
    if (!departure || !arrival) {
      return res
        .status(400)
        .json({ error: "Departure and arrival must be provided" });
    }
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "No airlines found for this route." });
    }
  ...
  try {
    ...
    if (!/^[A-Z0-9]{2}$/.test(airline)) {
      return res
        .status(400)
        .json({
          error: "Invalid airline format. Must be a 2-character IATA code.",
        });
    }
    const airlineExists = await pool.query(
      "SELECT * FROM airlines WHERE iata = $1",
      [airline]
    );
    if (airlineExists.rows.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid airline. Airline not found in database." });
    }
    ...
    if (!/^[A-Z]{3}$/.test(departure) || !/^[A-Z]{3}$/.test(arrival)) {
      return res
        .status(400)
        .json({
          error: "Invalid airport format. IATA codes must be 3 characters.",
        });
    }
    const depExists = await pool.query(
      "SELECT * FROM airports WHERE iata = $1",
      [departure]
    );
    const arrExists = await pool.query(
      "SELECT * FROM airports WHERE iata = $1",
      [arrival]
    );
    if (depExists.rows.length === 0 || arrExists.rows.length === 0) {
      return res
        ...
        .json({
          error:
            "Invalid departure or arrival airport. Airport not found in database.",
        });
    }
    ...
    if (!/^[A-Z0-9]{3}$/.test(planes)) {
      return res
        ...
        .json({
          error:
            "Invalid aircraft format. Must be a 3-character aircraft code.",
        });
    }
    const aircraftExists = await pool.query(
      "SELECT * FROM planes WHERE code = $1",
      [planes]
    );
    if (aircraftExists.rows.length === 0) {
      return res
        .status(400)
        .json({ error: "Invalid aircraft. Aircraft not found in database." });
    }
...
app.put("/routes", async (req, res) => {
  try {
    ...
    let aircraftList = route.rows[0].planes
      ? route.rows[0].planes.split(" ")
      : [];
...
/* -------------------- 6️⃣ Extra Credits -------------------- */
app.get('/update-airport/:iata/:icao', async (req, res) => {
  try {
    const { iata, icao } = req.params;
